...
### 소유자 1회 설정 — 하고 나면 데스크톱 앱과 같은 UX

데스크톱 앱이 OAuth 자격증명을 내장했던 것처럼, 아래 두 값을 저장소
**Settings → Secrets and variables → Actions → Variables 탭**에 만들어두면
빌드에 자동으로 심어집니다. 그 후 사용자(지인)는 **Drive는 "Google 로그인"
버튼만, Notion은 자기 토큰+페이지 ID만** 입력하면 됩니다. 두 값 모두 어차피
공개 번들에 들어가는 정보라 비밀이 아닙니다.

**① `GOOGLE_CLIENT_ID` 변수 — 5분**

[Google Cloud Console](https://console.cloud.google.com/apis/credentials)에서
**데스크톱 앱과 같은 프로젝트**에 OAuth 클라이언트 ID를 하나 더 만듭니다.
유형: **웹 애플리케이션**, "승인된 JavaScript 원본"에
`https://lde-rl2.github.io`를 추가. 생성된
`xxxx.apps.googleusercontent.com` 값을 저장소 변수 `GOOGLE_CLIENT_ID`로 저장.

- 같은 프로젝트여야 하는 이유: `drive.file` 권한은 "같은 앱(프로젝트)이 만든
  파일"만 보이기 때문입니다. 다른 프로젝트면 데스크톱이 올린 파일이 안 보입니다.

**② `NOTION_PROXY_URL` 변수 — 5분 (Notion 쓸 때만)**

Notion API는 브라우저 호출을 차단(CORS)하므로 얇은 중계 서버가 필요합니다.
[workers/notion-proxy.js](workers/notion-proxy.js)를 Cloudflare Workers 무료
플랜에 한 번 배포합니다 (토큰을 저장하지 않는 무상태 전달자이며, 지인 전원이
같은 프록시를 공유합니다):

```bash
npx wrangler login    # 브라우저에서 Cloudflare 로그인 (무료 가입)
npx wrangler deploy workers/notion-proxy.js --name aicore-notion-proxy \
  --compatibility-date 2026-07-01
```

출력된 `https://aicore-notion-proxy.<계정>.workers.dev` 주소를 저장소 변수
`NOTION_PROXY_URL`로 저장.

두 변수를 만든 뒤 아무 커밋이나 push(또는 Actions에서 Re-run)하면 반영됩니다.

### 사용자(지인) 입장에서의 연결 방법

- **Drive**: 설정 → 동기화 → Drive → **Google 로그인** 버튼. 끝.
  브라우저 토큰은 1시간짜리라 세션이 오래 끊기면 "다시 로그인" 안내가 뜰 수
  있습니다(데이터는 안전하며, 로그인하면 이어서 동기화됩니다).
- **Notion**: 데스크톱과 동일하게 자기 통합 토큰과 부모 페이지 ID 입력 →
  **저장 & 연결 테스트**.

변수를 설정하지 않은 빌드에서는 예전처럼 설정 화면에 클라이언트 ID/프록시
URL 입력칸이 나타납니다(직접 입력도 여전히 가능).
